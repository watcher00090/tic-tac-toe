          ...
          for (int i = 0; i < files.length; i++) {
            ...
            if (fileName != "driver.py" && getFileType(fileName).toLowerCase() == "py") {
              ...
              script {
                env.CONTAINER_ID = sh(
                  returnStdout: true,
                  script: "docker run -d -v build-$BUILD_ID-artifacts:/home/data/ --env ARTIFACTS_DATAPATH=/home/data --env CODE_PATH=/home/tic-tac-toe tic-tac-toe-test:build-$BUILD_ID python /home/tic-tac-toe/test/${files[i].name}"
                ).trim()
              }
